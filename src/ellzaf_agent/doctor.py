"""Repository coverage doctor for Ellzaf Agent instrumentation."""
...
def doctor_repo(path: str | Path, *, max_files: int = 1000) -> EllzafIntegrationReport:
...
def format_doctor_report(report: EllzafIntegrationReport) -> str:
    lines = [
        f"Ellzaf Agent doctor report for {report.project}",
        "",
        "Coverage:",
    ]
...
def _doctor_warnings(files: tuple[_DocFile, ...]) -> list[str]:
    ...
    if "ellzaf_agent" not in all_text and "ellzaf-agent" not in all_text:
        warnings.append("No existing Ellzaf Agent instrumentation found.")
